...
from config.config import merge_features, orig_rules, rules, rules_normal, all_mappings 
...
if not os.path.exists(fname):
    ...
    for k, df in tqdm(dict_dfs_processed.items()):
        ...
        if (df['Height'] is not None) and (df['Weight'] is not None):   
            ...
            # df Height fill with most frequent value
            # df['Height'].fillna(df['Height'].value_counts().index[0], inplace=True)
